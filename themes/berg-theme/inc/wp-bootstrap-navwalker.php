...
class wp_bootstrap_navwalker extends Walker_Nav_Menu {
    ...
    public function start_lvl( &$output, $depth = 0, $args = array() ) {
        ...
        $output .= "\n$indent<div class=\"mobile-drop\"></div><ul role=\"menu\" class=\" dropdown-menu level-".$depth." sub-menu\">\n";
    }
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
            ...
            */
            ...
            $item_output .= $args->link_before . $this->menu_image_nav_menu_item_title_filter(  $item->title, $item, $depth, $args) . $args->link_after;
...
//            $this->setUsedAttachments( $image_size, $item->thumbnail_hover_id );
        ...
        $menu_description_position   = $item->menu_description_position;
        $menu_description = wpautop( $item->menu_description);

        switch ( $menu_description_position ) {
            case 'hide':
            case 'before':
                $item_args = array( $none, $menu_description, $title );
                break;
            case 'after':
            default:
                $item_args = array( $title, $menu_description, $none );
                break;
        }
        if (trim($menu_description)) {
            $title = vsprintf('%s<span class="menu-description">%s</span>%s', $item_args);
        }
