...
<div class="bs-posts <?php echo $facetwp_additional_classes; ?>">
	<div class="bs-post__list row">
		...
		<?php while (have_posts()) : the_post();
            ...
            $categories = implode(" | ", wp_get_object_terms(get_the_ID(), 'resource-category',  array("fields" => "names")));
            ...
            $is_webinar_post    = has_term(['webinar', 'webinars', 'Webinar', 'Webinars'], 'resource-category', $post_id);
            $webinar_text       = '';
            if($is_webinar_post) {
                $webinar_status = get_post_meta($post_id, 'webinar_settings_status', true);
                $webinar_date_time = get_post_meta($post_id, 'webinar_settings_date_time', true);
                if ('upcoming' == $webinar_status) {
                    $webinar_text = $webinar_date_time;
                } else {
                    $webinar_text = 'On Demand';
                }
            }
		?>
					...
					if (isset($display_order)) {
						...
						foreach ($display_order as $order => $item) {
							switch ($item) {
                                ...
                                case 'webinar_text':
                                    if($webinar_text):
                                        echo '<div class="bs-post__webinar-text"><span>' . $webinar_text . '</span></div>';
                                    endif;
                                    break;
								default:
									break;
